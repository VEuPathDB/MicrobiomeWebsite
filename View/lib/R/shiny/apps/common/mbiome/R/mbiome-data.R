MicrobiomeData <- R6Class("MicrobiomeData",
      private = list(
       ...
       public = list(
         ...
         get_top_n_by_mean = function(taxonomy_level=NULL, n=10, use_relative_abundance=T, add_other=T){
           self$otu_table$get_top_n_by_mean(taxonomy_level, n, use_relative_abundance, add_other)
         },
         get_single_otu = function(taxonomy_level=NULL, otu_name, add_other=F, other_name="zzzother"){
           self$otu_table$get_single_otu(taxonomy_level, otu_name, add_other, other_name)
         }
       )
)
